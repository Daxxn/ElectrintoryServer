...
//         mongoUrl: env.parsed.DB_CONNECT,
//       }),
//       cookie: {
//         secure: false,
//         maxAge: 6000000,
...
const databaseConnection = (app, callback) => {
  const db = mongoose.connect(env.parsed.DB_CONNECT, {}, callback);
  apiRoutes(app, db);
  return db;
};
// #endregion
...
const start = () => {
  ...
  databaseConnection(app, (err) => {
    if (err) {
      ...
      server.listen(env.parsed.PORT, (err) => {
        if (err) {
          console.log(err);
        } else {
          console.log(`Started Server : ${env.parsed.PORT}`);
        }
      });
    }
  });
